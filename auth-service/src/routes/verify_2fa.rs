...
pub async fn verify_2fa(
    ...
    let code_tuple = match two_fa_code_store.get_code(&email).await {
        ...
        Err(_) => return (jar, Err(AuthAPIError::InvalidCredentials)), // 400: No 2FA code exists for email
    };

    if !code_tuple.0.eq(&login_attempt_id) || !code_tuple.1.eq(&two_fa_code) {
        return (jar, Err(AuthAPIError::IncorrectCredentials)); // 401: Wrong credentials provided
    }
