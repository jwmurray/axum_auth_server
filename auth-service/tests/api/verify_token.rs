...
async fn should_return_401_if_invalid_token() {
    ...
    let invalid_token = "invalid-token-string".to_string();

    let invalid_verify_token_body = serde_json::json!({
        "token": &invalid_token,
    });

    let response = app.post_verify_token(&invalid_verify_token_body).await;
